...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validaciones
    if (empty($descripcion) || $monto <= 0 || empty($fecha) || empty($method)) {
        ...
        $stmt->bind_param('sdss', $descripcion, $monto, $fecha, $method);
        
        if ($stmt->execute()) {
            $success = "Pago agregado exitosamente";
            // Limpiar formulario
            $descripcion = $monto = $fecha = $method = '';
        } else {
            $error = "Error al agregar el pago: " . $conexion->error;
        }
        $stmt->close();
    }
}
...
<body class="bg-gray-100 min-h-screen">
    ...
    <nav class="bg-indigo-700 rounded-b-2xl px-4 sm:px-8 py-4 shadow-lg relative z-10">
        ...
        <div id="nav-menu" class="flex-col sm:flex-row sm:flex items-center gap-6 sm:gap-8 mt-4 sm:mt-0 hidden sm:flex">
            ...
            <a href="addPago.php" class="px-6 py-2 rounded-lg bg-white text-indigo-700 font-semibold shadow hover:bg-indigo-100 focus:bg-indigo-100 transition block sm:inline-block">Agregar Pago</a>
            ...
            <form method="POST" class="space-y-6">
                <div>
                    ...
                    <input type="text" name="descripcion" required 
                           ...
                           placeholder="Ej: Pago DORADA, Finanzas, etc.">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="metodo" required class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-indigo-400">
                ...
                <div class="flex flex-col sm:flex-row justify-between gap-2">
                    ...
                    <button type="submit" class="px-6 py-2 bg-indigo-700 text-white rounded hover:bg-indigo-800 font-semibold w-full sm:w-auto">Agregar Pago</button>
                </div>
            </form>
        </div>
    </main>
